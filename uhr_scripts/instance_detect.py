...
if __name__ == "__main__":
    ...
    parser.add_argument('--end_index', type=int, default=510)
    parser.add_argument('--save_path', type=str, default='./', help='save the boxes to json')
    parser.add_argument('--data_dir', type=str, default='/data9/gyl/RS_DATASET/URUR/train')
    parser.add_argument('--select_boxes', type=int, default=50, help="select boxes for each image")
    ...
    with torch.no_grad():
        with jl.open(jsonl_file_path, 'a') as f:
            for i in tqdm(range(args.start_index, min(img_num, args.end_index))):
                ...
                rank_dict = get_rank(args.dataset)
